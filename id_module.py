...
# get intrusion detection results in real-time
# input: vnf_instance name prefix, VNF types
# Output: result (string) 
def get_intrusion_detection_result(prefix, sfc_vnfs):
...
def read_data(file_path):
    ...
    vnf1_abnormal_data = read_csv(file_path+'/attack/'+vnf1_file_list[0])
    for file_name in vnf1_file_list[1:]:
        data=read_csv(file_path+'/attack/'+file_name)
    ...
    vnf2_abnormal_data = read_csv(file_path+'/attack/'+vnf2_file_list[0])
    for file_name in vnf2_file_list[1:]:
        data=read_csv(file_path+'/attack/'+file_name)
...
def get_id_f1score():
    ...
    #train_y=F.one_hot(torch.tensor(train_y).to(device), num_classes=2).float()
    #m=nn.Sigmoid()
